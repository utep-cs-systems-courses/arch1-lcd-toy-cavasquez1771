...
void drawFish(u_char col, u_char row, u_char size, u_int colorBGR)
{
  u_char r;
  u_char c;
  //head of fish
  ...
  //body
  fillRectangle(col+size, row-size/3, size/1.5, size/1.5, colorBGR);

  for(r = size/2; r>0; r--){
    for(c = size/2; c >= r; c--){
      drawPixel(col + size*1.5 - c, row + size/4+r, colorBGR);
    }
  }
  ...
  //tail
  fillRectangle(col+size*1.5-2, row-size/3, size/1.5, size/1.5, colorBGR);
  ...
  //eye
  fillRectangle(col+size*0.7, row-size/2, size/6, size/6, COLOR_BLACK);
}

void drawDiamond(u_char col, u_char row, u_char size, u_int colorBGR){
  for(u_char r = 0; r < size; r++){
    for(u_char c = 0; c < r; c++){
      drawPixel(col-c, row-r-1, colorBGR);
      drawPixel(col-c, row+r-(2*size), colorBGR);
      drawPixel(col+c, row-r-1, colorBGR);
      drawPixel(col+c, row+r-(2*size), colorBGR);
    }
  }
     ...
     u_int fgColorBGR, u_int bgColorBGR) 
{
  u_char col = 0;
  u_char row = 0;
  u_int bit = 0x0001; //Changed u_char to u_int
  u_char oc = c - 0x20;
